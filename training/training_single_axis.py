...
from my_models.monai_densenet import monai_densenet_single_view
from evaluation import *


class training_single_axis():
    #set variables
    def __init__(self, batch_size, lr, optimizer, l1, l2, axis):
        ...
        self.axis = axis
        #read csv files
        train_set = pd.read_csv("dataframes/training_set_t2w.csv").reset_index(drop=True)
        train_set = train_set[(train_set["mri_type"] == "T2w")].reset_index(drop=True)
        test_set = pd.read_csv("dataframes/testing_set_t2w.csv").reset_index(drop=True)
        test_set = test_set[(test_set["mri_type"] == "T2w")].reset_index(drop=True)
        val_set = pd.read_csv("dataframes/validation_set_t2w.csv").reset_index(drop=True)
        ...
        self.model = monai_densenet_single_view(l1 = self.l1, l2 = self.l2)
    ...
    def train_single_view(self, num_epochs, train_dataloader, val_dataloader, plot=False):
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(train_dataloader):   
                self.model.train()
                input = batch["image"][self.axis].to(self.device, dtype=torch.float)
            ...
            if is_best:
                torch.save(self.model,"best_model_x_axes.pt" )
    ...
    def train_evaluate(self):
        ...
        # get_f1_score_precision_recall_single_input(model,self.dataloader_test, self.axis)
        # get_confusion_matrix_AUC_single_input(last_model, self.dataloader_test,self.axis, nb_classes=2)


        print("\n best model info")
        best_model = monai_densenet_single_view()
        best_model = torch.load("best_model_x_axes.pt")
        auc = get_confusion_matrix_AUC_single_input(best_model, self.dataloader_test,self.axis,nb_classes=2)
        f1_score, recall, precision = get_f1_score_precision_recall_single_input(best_model, self.dataloader_test, self.axis)
        ...
        # experiments.to_csv(file_name, index=False)
