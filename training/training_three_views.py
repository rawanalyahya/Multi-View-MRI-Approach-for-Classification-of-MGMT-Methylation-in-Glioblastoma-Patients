...
class train_three_view():
    def __init__(self, batch_size, lr, optimizer, l1, l2, loss_function):
        ...
        print("number of patient in training set = ", len(train_set.BraTS21ID.unique()))
        print("number of patient in test set = ", len(test_set.BraTS21ID.unique()))
        print("number of patient in val set = ", len(val_set.BraTS21ID.unique()))
        ...
        if self.loss_function == "AUCMLoss":
            ...
            self.criterion = nn.CrossEntropyLoss()#weight=torch.from_numpy(weight).float().to(self.device))

        assert self.optimizer in ["Adam", "SGD", "Adagrad", "PESG"]
    ...
    def train_evaluate(self):
        ...
        new_dir_path = "./results/"+str(auc_test)+"_F1_"+str(f1_score_test)
        os.mkdir(new_dir_path)
        new_results.to_csv("new_dir_path/experiment_summary.csv", index=False)
